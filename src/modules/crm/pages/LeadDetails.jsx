...
import { Phone, Calendar, User, MessageSquare, Clock, ArrowLeft, Save, MapPin, Calculator, CreditCard, Edit } from 'lucide-react';
...
const LeadDetails = ({ userProfile }) => {
    ...
    const [editingLead, setEditingLead] = useState(null); // Edit Mode State
        ...
        <div className="max-w-6xl mx-auto p-4 md:p-6 bg-gray-50 min-h-screen">
            ...
            <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
                ...
                <div className="bg-slate-800 p-6 text-white flex flex-col md:flex-row justify-between md:items-center gap-4">
                    ...
                    {/* EDIT BUTTON (Manager/Director/Owner) */}
                    <div className="flex items-center gap-3">
                        {(userProfile?.role === 'DIRECTOR' || userProfile?.role === 'MANAGER' || userProfile?.uid === lead.assignedTo || userProfile?.role === 'ACCOUNTANT') && (
                            <button
                                onClick={() => setEditingLead(lead)}
                                className="bg-white/10 hover:bg-white/20 border border-white/20 text-white px-4 py-2 rounded-lg font-bold flex items-center gap-2 transition"
                            >
