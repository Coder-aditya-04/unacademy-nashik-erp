...
export const assignLead = async (leadId, staffObj, assignedBy) => {
    try {
        ...
        const isConverted = hasAdmission || ['CONVERTED', 'TOKEN_PAID', 'ADMISSION_TAKEN', 'CLOSED', 'LOST', 'REJECTED', 'REFUNDED', 'UNDER_VERIFICATION'].includes(currentStatus);
        ...
        if (isConverted) {
            newStatus = ['TOKEN_PAID', 'ADMISSION_TAKEN', 'CLOSED', 'LOST', 'REJECTED', 'REFUNDED', 'UNDER_VERIFICATION'].includes(currentStatus) 
                ? currentStatus 
                : "CONVERTED";
        }
